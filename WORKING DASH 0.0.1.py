...
app.layout = html.Div(style={'backgroundColor':'#141414', 'color':'#E8E8E8'}, children = [
    ...
    html.Button('Train Model', id='start-button', n_clicks=0),
    dcc.Interval(
...
def start_data_generation(n_clicks, n_intervals):
    if n_clicks > 0:
        # Start the data generation function in a separate thread
        data_thread = threading.Thread(target=training_data_gen)
        data_thread.daemon = True  # Daemonize the thread so it stops when the main program exits
        data_thread.start()
    return n_intervals
...
torch.save(policy_net.state_dict(), r"C:\Python Projects\ML Training Files\training.pth")
